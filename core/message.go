...
type Message struct {
	...
	// ClientUserMessageID is the stable marker passed to agents that can persist
	// caller identity with a turn. It defaults to MessageID and is generated by
	// core only when the platform has no message identity.
	ClientUserMessageID string
	// ReferencedMessageID is the platform message directly replied to, when
	// available. It is identity metadata, not quoted display content; core uses
	// it to bind exact actions such as steering the turn shown by a card.
...
type Event struct {
	Type                     EventType
	ThreadID                 string // immutable backend thread identity, when available
	TurnID                   string // immutable backend turn identity, when available
	ItemID                   string // immutable backend item identity, when available
	ClientUserMessageID      string // caller marker persisted with the turn, when available
	Content                  string
